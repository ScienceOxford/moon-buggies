            ...
            try:
                ...
                if message[0] in directions:
                    try:
                        ...
                        mistake(3) # if a direction but not given both speed and time

                elif message[0] == 'grab':
                    if message[1] in grabber:
                        pin16.write_analog(grabber[message[1]])
                    else:
                        mistake(4) # if grab but not given correct option

                else:
                    mistake(2) # if not in directions or grab, you spelt something wrong

            except:
                mistake(1) # if message cannot be split - you need more info OR you spelt something wrong
